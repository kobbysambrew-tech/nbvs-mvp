...
function handlePermissionError(err, friendly) {
  ...
  showToast(friendly || "Permission problem (see console)", {
    type: "error",
    timeout: 6000
  });
}
...
async function initFirebase() {
  try {
    ...
    onAuthStateChanged(auth, async (user) => {
      ...
      try {
        // Read the user doc to get their role
        const uDoc = await getDoc(doc(db, "users", user.uid));
        if (!uDoc.exists()) {
          await signOut(auth);
          showToast("No user record found", { type: "error" });
          window.location.href = "/dashboard.html";
          return;
        }
        ...
        // Only allow superadmin/staff into this admin panel
        ...
        // Now that we know the role, bind UI + load data
        setupBindings();
        await loadInitialData();
      } catch (err) {
        console.error(err);
        showToast("Role check failed", { type: "error" });
        await signOut(auth);
        window.location.href = "/dashboard.html";
      }
    });
    ...
    console.error(err);
    showToast("Firebase init failed", { type: "error" });
  }
}
...
async function loadStats() {
  ...
  try {
    ...
    if (statSearches) statSearches.textContent = searchesSnap.size;
    if (statTransactions) statTransactions.textContent = txSnap.size;

    let total = 0;
    walletSnap.forEach((d) => (total += Number(d.data()?.balance || 0)));
    if (statWallet) statWallet.textContent = `$${total.toFixed(2)}`;
  } catch (err) {
...
async function saveNewRecord() {
  ...
  if (!name || !nia) {
    showToast("Name & NIA required", { type: "error" });
    return;
  }
  ...
  try {
    ...
    console.error(err);
    showToast("Cannot add record", { type: "error" });
  }
}
...
async function deleteRecord(id) {
  try {
    ...
    console.error(err);
    showToast("Cannot delete record", { type: "error" });
  }
}
...
async function loadSearchLogs() {
  ...
  try {
    ...
    snap.forEach((d) => {
      const row = d.data();
      const date = row.timestamp?.seconds
    ...
    handlePermissionError(err, "Cannot load search logs");
  }
}
...
async function adminVerify(mode) {
  ...
  if (!raw) {
    showToast("Enter value", { type: "error" });
    return;
  }
  ...
  try {
    const snap = await getDocs(
      query(collection(db, "records"), where(field, "==", raw), limit(1))
    );
    if (snap.empty) {
      output.innerHTML =
    ...
    output.innerHTML =
      "<div style='color:#b00020'>Error verifying</div>";
  }
}
...
function setupBindings() {
  ...
  $("#btn-verify-email")?.addEventListener("click", () =>
    adminVerify("email")
  );
  ...
  tabs.forEach((tab) => {
    $("#tab-" + tab)?.addEventListener("click", () => showSection(tab));
  });
...
function showSection(section) {
  const sections = ["dashboard", "verify", "users", "wallet", "logs", "audit"];
  sections.forEach((sec) =>
    $("#section-" + sec)?.classList.remove("active")
  );
...
document.addEventListener("DOMContentLoaded", () => {
  // Only init Firebase; bindings are wired once we know role
  initFirebase();
});
