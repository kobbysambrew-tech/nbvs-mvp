// admin-part1.js — Firebase init, auth, helpers, stats, wallet, search logs, audit logs
import { initializeApp } from "https://www.gstatic.com/firebasejs/10.11.0/firebase-app.js";
import { getAuth, onAuthStateChanged, signOut } from "https://www.gstatic.com/firebasejs/10.11.0/firebase-auth.js";
import {
  getFirestore, collection, doc, getDoc, getDocs, query, orderBy, limit, startAfter, where
  ...
  try {
    ...
    onAuthStateChanged(auth, async user => {
      ...
      try{
        ...
        console.log("User signed in:", user.uid, "Role:", role);
...
// -------------------- SIGN OUT --------------------
export async function doSignOut() { if(!auth) return; try{ await signOut(auth); showToast("Signed out"); window.location.href="/dashboard.html"; } catch(err){ showToast("Sign out failed",{type:"error"}); } }
...
export async function loadAuditLogs(){
  ...
  try{
    ...
    snap.forEach(d=>{
      ...
      const date=row.timestamp?.seconds?new Date(row.timestamp.seconds*1000).toLocaleString():"";
      const tr=document.createElement("tr");
      tr.innerHTML=`<td>${escapeHtml(date)}</td><td>${escapeHtml(row.user||"—")}</td><td>${escapeHtml(row.action||"—")}</td><td>${escapeHtml(row.details||"—")}</td>`;
      tbody.appendChild(tr);
    });
  } catch(err){ handlePermissionError(err,"Cannot load audit logs"); }
}
// admin-part2.js — Users/Records, Verify, UI bindings
import { db, currentUserRole } from './admin-part1.js';
import { collection, getDocs, addDoc, deleteDoc, query, where, limit } from "https://www.gstatic.com/firebasejs/10.11.0/firebase-firestore.js";
import { showToast, escapeHtml, $ } from './admin-part1.js';
  ...
  try {
    ...
    snap.forEach(s => {
      ...
      tr.innerHTML = `
        ...
        <td>${currentUserRole === "superadmin" ? `<button class="delete-btn btn-danger btn-small" data-id="${s.id}">Delete</button>` : ""}</td>
      `;
      tbody.appendChild(tr);
...
export async function deleteRecord(id) {
  try { await deleteDoc(collection(db, "records").doc(id)); showToast("Record deleted"); loadUsers(); }
